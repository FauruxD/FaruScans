    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="mt-4 grid gap-6 lg:grid-cols-[300px_1fr]">
        <div className="mx-auto w-full max-w-xs lg:max-w-none">
          <div className="relative aspect-[3/4.2] overflow-hidden rounded-lg border border-white/10 bg-zinc-900">
            {comic.thumbnail ? (
              <Image
                src={comic.thumbnail}
        ...
        <div className="space-y-5">
          <div>
            <div className="mb-3 flex flex-wrap gap-2">
              {toArray(comic.genres).map((genre) => (
                <span
                  key={genre}
                  className="rounded-md bg-cyan-300/10 px-2 py-1 text-xs font-semibold text-cyan-200"
                >
                  {genre}
                </span>
              ))}
            ...
            {comic.alternativeTitle ? (
              <p className="mt-2 text-sm text-zinc-400">{comic.alternativeTitle}</p>
            ) : null}
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {latestNumber ? (
              <Link
                ...
                className="flex h-12 items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 text-sm font-bold text-white transition hover:bg-white/10"
              >
          ...
          {infoEntries.length ? (
            <div className="grid gap-2 rounded-lg border border-white/10 bg-zinc-900/60 p-4 sm:grid-cols-2">
              {infoEntries.map(([key, value]) => (
                <div key={key} className="rounded-md bg-white/[0.03] p-3">
      ...
      <section className="mt-10 grid gap-8 lg:grid-cols-[1fr_320px]">
        ...
        <aside className="rounded-lg border border-white/10 bg-zinc-900/60 p-4">
          <h2 className="mb-3 font-bold text-white">Ringkasan</h2>
          <div className="space-y-3 text-sm text-zinc-400">
            <p className="flex items-center justify-between gap-4">
              <span>Total chapter</span>
              <span className="font-bold text-zinc-100">{chapters.length}</span>
            </p>
            {latestNumber ? (
              <Link
                href={`/baca/${slug}/${latestNumber}`}
                className="flex items-center justify-between rounded-lg bg-white/5 p-3 font-semibold text-zinc-100 transition hover:bg-white/10"
              >
